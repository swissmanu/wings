...
define(function(require, exports, module) {
	...
	var DefaultStrategy = function DefaultStrategy(canvasWrapper) {
		...
		_self.mouseMoved = function mouseMoved(absolutePosition) {
			...
			if(deepestWidget) {
				var moveEvent = deepestWidget.createDispatchableEvent('mouse:move', {absolutePosition:absolutePosition});
				deepestWidget.emit('dispatch', moveEvent);
				
				
				if(_mouseOverWidgets.indexOf(deepestWidget) === -1) {
					var enterEvent = deepestWidget.createDispatchableEvent('mouse:enter', {absolutePosition: absolutePosition});
					_mouseOverWidgets.push(deepestWidget);
					deepestWidget.emit('dispatch', enterEvent);
				}
			}
			...
			canvasWrapper.searchMouseWidgetsOnPosition(absolutePosition,undefined
				,function notFound(widget) {
					...
					if(widget !== deepestWidget && index > -1) {
						var exitEvent = widget.createDispatchableEvent('mouse:exit', {absolutePosition: absolutePosition});
		...
		_self.mouseButtonPressed = function mouseButtonPressed(button, absolutePosition) {
			...
			if(deepestWidget) {
				var downEvent = deepestWidget.createDispatchableEvent('mouse:down', {button:button, absolutePosition: absolutePosition});
				deepestWidget.emit('dispatch', downEvent);
			}
		};
		...
		_self.mouseButtonReleased = function mouseButtonReleased(button, absolutePosition) {
			...
			if(deepestWidget) {
				var upEvent = deepestWidget.createDispatchableEvent('mouse:up', {button:button, absolutePosition: absolutePosition});
				deepestWidget.emit('dispatch', upEvent);
			}
		};
		...
		_self.mouseButtonClicked = function mouseButtonClicked(button, absolutePosition) {
			...
			if(deepestWidget) {
				var clickEvent = deepestWidget.createDispatchableEvent('mouse:click', {button:button, absolutePosition: absolutePosition});
				deepestWidget.emit('dispatch', clickEvent);
			}
		};
