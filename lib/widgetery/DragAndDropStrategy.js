...
define(function(require, exports, module) {
	...
	var DragAndDropStrategy = function DragAndDropStrategy(canvasWrapper) {
		...
		_self.dragStarted = function dragStarted(absolutePosition) {
			 ...
			 */
			initiatorCandidate.emit('native_dragStart', absolutePosition, acceptCallback, cancelCallback);
		};
				...
				_self.getCanvasWrapper().searchMouseWidgetsOnPosition(absolutePosition, 
					function found(widget) {
						if(_mouseOverWidgets.indexOf(widget) === -1) {
							 ...
							 */
							widget.emit('native_mouseDragEntered', _initiator, _data);
						}
						widget.emit('native_mouseDrag', absolutePosition, _initiator, _data);
						...
						if(index > -1) {
							 ...
							 *                     transfered with the DnD process.
							 */
							widget.emit('native_mouseDragExited', _initiator, _data);
						}
						widget.emit('native_mouseDrag', absolutePosition, _initiator, _data);
					}
				);				
		...
		_self.mouseButtonReleased = function mouseButtonReleased(button, absolutePosition) {
			...
			if(_initiator) _initiator.emit('native_dragStop', absolutePosition);
			if(dropTarget !== _initiator) dropTarget.emit('native_dragStop', absolutePosition);
		...
		function stopDrag(absolutePosition) {
			 ...
			 *                                 stopped.
			 */
			_self.emit('stopDrag', absolutePosition);
		}
